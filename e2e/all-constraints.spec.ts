...
test.describe('All Constraint Types', () => {
  test('fix-x and fix-y constraints (point anchoring)', async ({ page }) => {
    ...
    // Anchor the point (creates fix-x and fix-y constraints)  
    await helper.anchorPoint(pointA);

    // Verify fix constraints were created
    await helper.expectConstraintExists('fix x');
    await helper.expectConstraintExists('fix y');
  ...
  test('distance constraint between two points', async ({ page }) => {
    ...
    const actualDistance = Math.sqrt(Math.pow(pB.x - pA.x, 2) + Math.pow(pB.y - pA.y, 2));
    expect(Math.abs(actualDistance - 150)).toBeLessThan(0.01);

    // Edit the constraint value to 100
    await helper.selectTool('select');
    await page.click('[data-testid="constraint-panel"] span:has-text("150.000")');
    ...
    const newDistance = Math.sqrt(Math.pow(newPB.x - newPA.x, 2) + Math.pow(newPB.y - newPA.y, 2));
  ...
  test('horizontal line constraint', async ({ page }) => {
    ...
    const positions = await helper.getPointPositions();
    const points = Object.values(positions);
    console.log(`Point 1 Y: ${points[0].y.toFixed(3)}, Point 2 Y: ${points[1].y.toFixed(3)}`);
  ...
  test('vertical line constraint', async ({ page }) => {
    ...
    const positions = await helper.getPointPositions();
    const points = Object.values(positions);
    console.log(`Point 1 X: ${points[0].x.toFixed(3)}, Point 2 X: ${points[1].x.toFixed(3)}`);
  ...
  test('perpendicular lines constraint', async ({ page }) => {
    ...
    console.log(`Slope 1: ${slope1.toFixed(3)}, Slope 2: ${slope2.toFixed(3)}, Product: ${(slope1 * slope2).toFixed(3)}`);
  ...
  test('constraint editing and canvas manipulation workflow', async ({ page }) => {
    const helper = new GeoCalcTestHelper(page);
    await helper.goto();
    ...
    await page.click('[data-testid="constraint-panel"] span:has-text("100.000")');
