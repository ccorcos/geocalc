...
test.describe("Canvas Context Menu Constraint Creation", () => {
	test("right-click on canvas with selected points shows constraint options", async ({
		page,
	}) => {
		...
		await expect(
			page.getByRole("button", { name: "X Distance", exact: true })
		).toBeVisible()
		await expect(
			page.getByRole("button", { name: "Y Distance", exact: true })
		).toBeVisible()
	})

	test("create distance constraint from canvas context menu", async ({
		page,
	}) => {
		const helper = new TestHarness(page)
		await helper.goto()
