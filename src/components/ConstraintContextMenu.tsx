...
export const ConstraintContextMenu: React.FC<ConstraintContextMenuProps> = ({
  ...
  const handleCreateConstraint = (constraintType: ConstraintType) => {
    ...
    const needsValue = getAvailableConstraints().find(c => c.type === constraintType)?.needsValue;
    ...
    if (needsValue) {
      ...
      const constraint = createConstraint(constraintType, selectedIds, undefined);
  ...
  const handleCreateConstraintWithValue = () => {
    ...
    const constraint = createConstraint(pendingConstraint.type, selectedIds, value);
    addConstraint(constraint);
      ...
      {!showValueDialog && (
        ...
        >
          ...
          {availableConstraints.length === 0 && (
            <div style={{ padding: "6px 12px", color: "#666", fontSize: "12px" }}>
      ...
      {showValueDialog && pendingConstraint && (
        ...
        >
          <div style={{ marginBottom: "8px", fontSize: "12px", color: "#666" }}>
            Enter value for {getAvailableConstraints().find(c => c.type === pendingConstraint.type)?.label}:
          </div>
          <input
            type="number"
            value={inputValue}
