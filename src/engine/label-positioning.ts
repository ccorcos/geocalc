 ...
 */
export function calculateLabelPosition(label: Label, geometry: Geometry): Position | null {
	switch (label.type) {
		case "coordinate": {
...
export function calculateDimensionLineEndpoints(
...
): { start: Position; end: Position; extensionLines: { p1Start: Position; p1End: Position; p2Start: Position; p2End: Position } } {
	...
	const extensionLength = 10 // 10px extension beyond dimension line
	const extensionLines = {
		p1Start: {
			x: p1.x - perpUnit.x * Math.sign(perpDistance) * extensionLength,
			y: p1.y - perpUnit.y * Math.sign(perpDistance) * extensionLength
		},
		p1End: {
			x: dimensionStart.x + perpUnit.x * Math.sign(perpDistance) * extensionLength,
			y: dimensionStart.y + perpUnit.y * Math.sign(perpDistance) * extensionLength
		},
		p2Start: {
			x: p2.x - perpUnit.x * Math.sign(perpDistance) * extensionLength,
			y: p2.y - perpUnit.y * Math.sign(perpDistance) * extensionLength
		},
		p2End: {
			x: dimensionEnd.x + perpUnit.x * Math.sign(perpDistance) * extensionLength,
			y: dimensionEnd.y + perpUnit.y * Math.sign(perpDistance) * extensionLength
		}
	}
